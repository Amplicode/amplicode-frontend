 ...
 * Collection of items
 */
function TableSection({ items, loading, error, selectedRowId, setSelectedRowId }: TableSectionProps) {
  if (loading) {
    return <Spin />;
  }
  ...
  return (
    <Space direction="vertical" style={{ width: "100%" }}>
      ...
      <Table
          ...
          columns={columns}
          rowClassName={record => (record as ItemType)?.id === selectedRowId ? 'table-row-selected' : ''}
          onRow={data => {return {onClick: () => setSelectedRowId((data as ItemType)?.id)}} }
      />;
    </Space>
  );
}
