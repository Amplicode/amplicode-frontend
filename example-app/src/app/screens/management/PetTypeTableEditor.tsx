...
export interface PetTypeTableEditorProps<TData = any> {
  /**
...
export function PetTypeTableEditor({
  refetchQueries
}: PetTypeTableEditorProps<QueryResultType>) {
  const intl = useIntl();
  useBreadcrumbItem(intl.formatMessage({ id: "screen.PetTypeTableEditor" }));
  ...
  return (
    <>
      ...
      <Form.Item
        ...
        help={
          <FieldErrorMessages path="defenseStatus" fieldErrors={fieldErrors} />
        }
        validateStatus={
          hasError(fieldErrors, "defenseStatus") ? "error" : "success"
        }
      >
        <Select>
          <Select.Option value="NEEDS_PROTECTION">
            Needs protection
          </Select.Option>
          <Select.Option value="NO_DANGER">No danger</Select.Option>
          <Select.Option value="RED_BOOK">Red book</Select.Option>
        </Select>
      </Form.Item>
    </>
  );
