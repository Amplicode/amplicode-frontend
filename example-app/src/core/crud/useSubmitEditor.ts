...
export function useSubmitEditor<TData>(
  ...
  const handleGraphQLError = useCallback(
    (errors: ReadonlyArray<GraphQLError>) => {
      ...
      setFiledErrors();
      setFormError(
        globalErrors.map((error: GraphQLError) => error.message).join("\n")
      );
      if (
        fieldErrors.length > 0 ||
        globalErrors.some((error: GraphQLError) => isBeanValidation(error))
      ) {
        return message.error(
          intl.formatMessage({ id: "common.validationError" })
        );
      } else if (
