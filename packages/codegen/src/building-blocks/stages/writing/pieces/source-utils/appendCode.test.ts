...
describe('appendCode()', () => {
  it('appends code to the end of source', () => {
    expect(appendCode(initial, codeToAppend)).to.eq(expectedResult);
  });
})

const initial = `export const GRAPHQL_URI = import.meta.env.VITE_GRAPHQL_URI ?? "/graphql";`

const codeToAppend = `export const LOGIN_URI = import.meta.env.VITE_LOGIN_URI ?? "/login";
...
const expectedResult = `export const GRAPHQL_URI = import.meta.env.VITE_GRAPHQL_URI ?? "/graphql";
